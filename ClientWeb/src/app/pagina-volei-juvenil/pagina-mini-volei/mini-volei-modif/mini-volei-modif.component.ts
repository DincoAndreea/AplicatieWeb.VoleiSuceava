...
@Component({
  selector: 'app-mini-volei-modif',
  templateUrl: './mini-volei-modif.component.html',
  styleUrls: ['./mini-volei-modif.component.css']
})
export class MiniVoleiModifComponent implements OnInit {
  @Input() lot: string = ``;
  ...
  Modifica(lot: string, informatii: string, imagine: string, event: any, id: number, imagi: string){
    ...
    //let table = document.querySelector("#body-min")?.firstElementChild?.children[4].firstElementChild?.querySelector('table');
    ...
    for(let i = 1; i < l; i++)
    {
      ...
      this.premiiMin.push({'id': id as number, 'denumirePremiu': a1, 'loc': a2, 'an': a3});
    }
    this.minivoleiUpdate!.id = id1 as number;
    this.minivoleiUpdate.imagineLot = imag1;
    this.minivoleiUpdate.informatii = info1;
    this.minivoleiUpdate.lot = lot1 as string;
    this.minivoleiUpdate.premiiMinivolei = this.premiiMin;
    
    if(this.checkLot(this.minivoleiUpdate.lot) && (this.checkPremii(this.premiiMin) || this.premiiMin.length == 0))
    {
      this.getMinivolei.updateMinivolei(this.minivoleiUpdate).subscribe();
      alert("Lotul de mini volei a fost modificat cu succes.");
    }
    else
    {
      if(this.checkLot(this.minivoleiUpdate.lot) == false)
      {
        alert("Lotul este invalid. Lotul trebuie să conțină cel puțin 10 caractere.");
      }
      if(this.checkPremii(this.premiiMin) == false)
      {
  ...
  Sterge(miniv: IMinivolei){
    if(this.getMinivolei.deleteMinivolei(miniv).subscribe())
    {
      alert("Lotul de mini volei a fost șters cu succes.");
    }
    else
    {
      alert("Nu s-a putut șterge lotul de mini volei.");
    }
