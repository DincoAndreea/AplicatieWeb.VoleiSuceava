...
import { isEmpty, Observable, Subject, VirtualTimeScheduler } from 'rxjs';
...
@Component({
  selector: 'app-cadeti-modif',
  templateUrl: './cadeti-modif.component.html',
  styleUrls: ['./cadeti-modif.component.css']
})
export class CadetiModifComponent implements OnInit {
  @Input() lot: string = ``;
  ...
  Modifica(lot: string, informatii: string, imagine: string, event: any, id: number, imagi: string){
    ...
    //let table = document.querySelector("#body-cad")?.firstElementChild?.children[4].firstElementChild?.querySelector('table');
    ...
    for(let i = 1; i < l; i++)
    {
      ...
      this.premiiCad.push({'id': id as number, 'denumirePremiu': a1, 'loc': a2, 'an': a3});
    }
    
    this.cadetiUpdate!.id = id1 as number;
    this.cadetiUpdate.imagineLot = imag1;
    this.cadetiUpdate.informatii = info1;
    this.cadetiUpdate.lot = lot1 as string;
    this.cadetiUpdate.premiiCadeti = this.premiiCad;
    if(this.checkLot(this.cadetiUpdate.lot) && (this.checkPremii(this.premiiCad) || this.premiiCad.length == 0))
    {
      this.getCadeti.updateCadeti(this.cadetiUpdate).subscribe();
      alert("Cadeții au fost modificați cu succes.");
    }
    else
    {
      if(this.checkLot(this.cadetiUpdate.lot) == false)
      {
        alert("Lotul este invalid. Lotul trebuie să conțină cel puțin 10 caractere.");
      }
      if(this.checkPremii(this.premiiCad) == false)
      {
  ...
  Sterge(cadet: ICadeti){
    if(this.getCadeti.deleteCadeti(cadet).subscribe())
    {
      alert("Cadeții au fost șterși cu succes.");
    }
    else
    {
      alert("Nu s-au putut șterge cadeții.");
    }
  }
